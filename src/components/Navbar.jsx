...
const BrandLogo = React.memo(() => (
  <Link
    ...
    aria-label="MentorWise Home"
  >
    <img src="/Compass.png" alt="" className="w-10 h-10 object-contain" />
...
/* --------------------- Mobile MegaMenu (Categories) --------------------- */
const MobileMegaMenuSubtopic = React.memo(
  ({ subtopic, categoryLabel, closeMenu }) => {
    const subName = subtopic?.name || subtopic || "";
    const to =
      subtopic?.path ||
      `/mentors?category=${encodeURIComponent(
        categoryLabel
      )}&subtopic=${encodeURIComponent(subName)}`;
    return (
      <Link
        to={to}
...
const MobileMegaMenuTopic = React.memo(
  ({ topic, categoryLabel, closeMenu }) => {
...
const MobileMegaMenuCategory = React.memo(
  ({ category, index, openIndex, setOpenIndex, closeMenu }) => {
...
const MobileHowItWorks = React.memo(({ onClose }) => {
  ...
  return (
    <div className="border-b border-gray-800 pb-2">
      <button
        ...
        onClick={() => setOpen((s) => !s)}
        aria-expanded={open}
        aria-controls="howitworks-accordion"
      >
        How It Works
        <ChevronDownIcon
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            className="pl-2 pr-1 overflow-hidden"
          >
            <ul className="mt-1 space-y-1.5">
              {HOW_IT_WORKS_SUB_LINKS.map((l) => (
                <li key={l.path}>
                  <Link
                    to={l.path}
                    onClick={onClose}
                    className="block rounded-md px-2 py-2 text-sm text-gray-200 hover:text-orange-400"
                  >
                    {l.label}
                  </Link>
                </li>
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          />
          <motion.aside
            className="fixed top-0 right-0 w-80 max-w-[88vw] h-full bg-gray-900 text-white z-[999] shadow-2xl flex flex-col"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {/* Primary links (minus How It Works, since we provide its accordion) */}
              <nav className="space-y-1" aria-label="Primary">
                <MobileHowItWorks onClose={onClose} />

                {NAV_LINKS.filter((l) => !l.dropdown).map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                    className="block font-medium hover:text-orange-400 py-2"
                    onClick={onClose}
                  >
                    {link.label}
                  </Link>
              ...
              {/* Browse Categories (mega menu data) */}
              <div>
                ...
                <ul className="space-y-1">
                  {megaMenuData.map((cat, idx) => (
                    <MobileMegaMenuCategory
                      key={cat?.name || idx}
                      category={cat}
                      index={idx}
                      openIndex={openCategoryIdx}
                      setOpenIndex={setOpenCategoryIdx}
                      closeMenu={onClose}
                    />
                  ))}
                </ul>
                ...
                {!user ? (
                  <div className="flex items-center gap-4">
                    <Link
                      to="/signin"
                      onClick={onClose}
                      className="flex-1 text-center px-4 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 font-semibold"
                    >
                      Sign In
                    </Link>
                    <Link
                      to="/signup"
                      onClick={onClose}
                      className="flex-1 text-center px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-700 text-white font-semibold"
                    >
                      Sign Up
                    </Link>
                  </div>
...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className="bg-gray-900 px-4 md:px-6 h-20 w-full overflow-x-clip flex justify-between items-center sticky top-0 z-[999] shadow-lg"
        role="navigation"
        aria-label="Main"
      >
        <BrandLogo />
      ...
      {/* Desktop mega menu bar (unchanged) */}
