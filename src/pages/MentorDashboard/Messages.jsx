...
// simple in-memory cache for /users/{uid}
const cache = new Map();
async function getProfile(uid) {
  ...
  const prof = { uid, name: d.name || d.displayName || "User", photoURL: d.photoURL || AVATAR };
  cache.set(uid, prof);
  return prof;
}
...
export default function MentorMessages() {
  ...
  const [people, setPeople] = useState({}); // uid -> {name, photoURL}
  ...
  // subscribe to my threads (left column)
  useEffect(() => {
    if (!me?.uid) return;
  ...
  async function send(e) {
    ...
    await addDoc(collection(db, "threads", activeId, "messages"), {
      text: clean, senderId: me.uid, createdAt: serverTimestamp(), readBy: [me.uid],
    });
  ...
  return (
    <div className="min-h-screen bg-orange-50 p-6">
      ...
      <p className="mb-6 text-slate-600">
        Keep in touch with your mentees. You can reply to questions, send updates, and follow up after sessions.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <aside className="md:col-span-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {!me?.uid ? (
            <div className="p-4 text-slate-500">Sign in to view messages.</div>
          ) : filtered.length === 0 ? (
        ...
        <section className="md:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[65vh]">
              ...
              msgs.map(m => {
                ...
                return (
                  <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[72%] px-4 py-2 rounded-2xl shadow ${mine ? "bg-orange-100 text-orange-900" : "bg-slate-100 text-slate-800"}`}>
                      {m.text}
          ...
          <form onSubmit={send} className="p-3 border-t border-slate-200 flex gap-2">
            ...
            />
            <button type="submit" className="px-5 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold shadow transition">
              Send
            </button>
          </form>
        </section>
